...
| [installer/BcContainerCreator.iss](../installer/BcContainerCreator.iss) | `#define MyAppVersion` |
| [build/build-installer.ps1](../build/build-installer.ps1) | `[string] $Version` (Default für lokale Bauten) |

Tipp: einfach nach der bisherigen Versionsnummer suchen und alle Treffer
ersetzen.
...
- **`dotnet format --verify-no-changes` rot**: lokal `dotnet format`
  laufen lassen und committen. CRLF vs. LF ist das häufigste Symptom — die
  `.editorconfig` schreibt CRLF vor.
...
Solange die App Phase-1-Status hat (`docs/ROADMAP.md`), bleiben wir bei
Patch-Bumps und vermeiden Major-Sprünge.
